    ...
    drawEarthFeatures() {
        ...
        for (let index = 0; index < this.satellites.length; index++) {
            ...
            if (mainWindow.satellites[index].latLong === undefined) {
                ...
                if (mainWindow.satellites[index].denseHistory !== undefined) {
                    mainWindow.satellites[index].latLong = mainWindow.satellites[index].denseHistory.map(h => {
                        let time = new Date(mainWindow.startDate - (-1000*h.t))
                        let eci = Object.values(getCurrentInertial(index, h.t))
                        let latLong = astro.eci2latlong(eci.slice(0,3), time)
                        return {lat: latLong.lat*180/Math.PI, long: latLong.long*180/Math.PI}
                    })
...
class Satellite {
    ...
    drawTrajectory() {
        ...
        if (this.denseHistory !== undefined) {
            mainWindow.drawCurve(this.denseHistory.map(s => s.position), {color: this.color})
            return 
        }
...
function changeSatelliteInputType(el) {
    ...
    switch (el.id) {
        ...
        case 'geo-sat-input':
            ...
            satInputs[5].innerHTML = `Arg Latitude <input class="sat-input" style="font-size: 1.25em; width: 10ch;" type="Number" placeholder="0"></div> deg`
...
function parseState(button) {
    ...
    if (tleCheck) {
        ...
        try {
            ...
            eciValues = {
                date: state[0],
                state: astro.teme2eci(state.slice(1,4), state.slice(4), state[0])//position
            }
            console.log(eciValues.state);
        }
        catch (e) {
...
function importSelectedEditSatellite(el) {
    ...
    switch (radioId) {
        case 'ric-sat-input':
            // use current state
            let originCurrentAtStart = propToTime(stateCurrent, -mainWindow.scenarioTime)
            ricState = Eci2Ric(originStart.slice(0,3), originStart.slice(3), originCurrentAtStart.slice(0,3), originCurrentAtStart.slice(3))
        ...
        case 'geo-sat-input':           
            ...
            satInputs[5].innerHTML = `Arg Latitude <input class="sat-input" style="font-size: 1.25em; width: 10ch;" type="Number" placeholder="${(coeStateStart.arg*180/Math.PI).toFixed(3)}"> deg</div>`
            break
    }   
}
...
function draw3dScene(az = azD, el = elD) {
    ...
    mainWindow.satellites.forEach((sat, satIi) => {
        ...
        if (!sat.locked) {
            ...
            if (sat.appear !== undefined) points.push(...sat.denseHistory.filter(s => s.t > mainWindow.scenarioTime).map(point => {
            ...
            else points.push(...sat.stateHistory.filter(s => s.t > mainWindow.scenarioTime).map(point => {
                let pos = math.multiply(r, [point.position[0], point.position[1], point.position[2]])
                return {
                    color: sat.color,
                    position: pos,
...
function calcSatTrajectoryDense(position = mainWindow.originOrbit, options = {}) {
    // Calculate dense trajetory, developed for launches
    let {timeDelta = mainWindow.timeDelta/20, tFinal = mainWindow.scenarioLength*3600, tStart = 0} = options
    let epochPosition = {...position}, epochTime = 0
    let propPosition, propOriginPosition, ricPosition, tProp = tStart, stateHist = []
    while ((tProp+timeDelta) < tFinal) {
        propPosition = propToTimeAnalytic(epochPosition, tProp-tStart)
        propOriginPosition = propToTimeAnalytic(mainWindow.originOrbit, tProp)
        ricPosition = Eci2Ric(propOriginPosition.slice(0,3), propOriginPosition.slice(3), propPosition.slice(0,3), propPosition.slice(3))
        stateHist.push({
            t: tProp, position: math.squeeze([...ricPosition.rHcw, ...ricPosition.drHcw])
        })
        
        tProp += timeDelta
    }
    propPosition = propToTimeAnalytic(epochPosition, tFinal-tStart)
    propOriginPosition = propToTimeAnalytic(mainWindow.originOrbit, tFinal)
    ricPosition = Eci2Ric(propOriginPosition.slice(0,3), propOriginPosition.slice(3), propPosition.slice(0,3), propPosition.slice(3))
    stateHist.push({
        t: tFinal, position: math.squeeze([...ricPosition.rHcw, ...ricPosition.drHcw])
    })
    return stateHist
}
